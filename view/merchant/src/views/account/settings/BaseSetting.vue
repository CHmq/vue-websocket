    ...
    <a-row :gutter="16">
      ...
      <a-col :md="24" :lg="8" :style="{ minHeight: '180px' }">
        <div class="ant-upload-preview" @click="$refs.modal.edit(1)" >
          ...
          <img :src="option.img !== `${this.$store.state.userUrl}` ? option.img : `${this.$store.state.userUrl}/agent/avatar2.jpg`"/>
        </div>
      </a-col>
...
export default {
  ...
  methods: {
    ...
    getInfo () {
      agentInfo().then((res) => {
        this.option.img = `${this.$store.state.userUrl}${res.result.avatar}`
        this.setInfo = res.result
        this.userAvatar = res.result.avatar
        this.changeInfoName(res.result.name)
        this.changeInfoAvatar(res.result.avatar)
        sessionStorage.setItem('userName', res.result.name)
        sessionStorage.setItem('userAvatar', res.result.avatar)
      })
    },
    // 更新个人信息接口
    editInfo (values) {
