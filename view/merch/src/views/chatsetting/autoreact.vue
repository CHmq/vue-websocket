  ...
  <div v-if="is_ok" class="wrap">
    <div class="item-box">
      ...
      <div class="content-box">
        <div class="item-content-box">
          <a-textarea :value="merch_welcome_msg.values.content" @change="toChangeInput(1)" ref="merch_welcome_msg_content" :disabled="is_can_edit_1===false"/>
          <!-- <quill-editor
        ...
        <div class="item-content-box content-input-box">
          ...
          <div>
            ...
            <a-input-number
              ...
              :min="20"
              :max="120"
              v-model="agent_countdown"
              ref="agent_countdown"
              :disabled="is_can_edit_2 === false"
              @change="toChangeInput(3)"/>
            <!-- <input class="item-input" @change="toChangeInput(3)" :value="agent.values.countdown" ref="agent_countdown" :disabled="is_can_edit_2===false"/> -->
            <span>秒后无响应自动发送</span>
          </div>
        </div>
        <div class="item-content-box">
          <a-textarea :value="agent.values.content" @change="toChangeInput(2)" ref="agent_content" :disabled="is_can_edit_2===false"/>
        </div>
      </div>
    </div>
    <div class="item-box">
        ...
        <div class="item-content-box content-input-box">
          ...
          <div>
            ...
            <a-input-number
              ...
              :min="20"
              :max="120"
              v-model="client_countdown"
              ref="client_countdown"
              :disabled="is_can_edit_3 === false"
              @change="toChangeInput(4)"/>
            <!-- <input class="item-input" @change="toChangeInput(4)" :value="client.values.countdown" ref="client_countdown" :disabled="is_can_edit_3===false"/> -->
            <span>秒后无响应自动发送</span>
          </div>
        </div>
        <div class="item-content-box">
          <a-textarea :value="client.values.content" @change="toChangeInput(5)" ref="client_content" :disabled="is_can_edit_3===false"/>
        </div>
      </div>
    </div>
    <div class="item-box">
    ...
    <div class="item-box">
      ...
      <div class="content-box">
        ...
        <div class="item-content-box">
          <a-textarea :value="dialog_end.values.agent_ending_message" @change="toChangeInput(6)" ref="agent_ending_message_content" :disabled="is_can_edit_5===false"/>
        </div>
      </div>
      <div class="content-box">
        <div class="item-content-box content-input-box"><span class="item-content-title">系统自动关闭时：</span></div>
        <div class="item-content-box">
          <a-textarea :value="dialog_end.values.auto_ending_message" @change="toChangeInput(7)" ref="auto_ending_message_content" :disabled="is_can_edit_5===false"/>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    toChangeInput (val) {
      if (val === 1) {
        ...
        this.client.countdown = this.client_countdown
        this.client.values.countdown = this.client_countdown
      } else if (val === 5) {
    ...
    onChange (val) {
      ...
      getMerchSettingList(postData).then((res) => {
        ...
        if (val === 1) {
          ...
          if (that.client.values.status === 0) {
    ...
    toCancel (val) {
      ...
      getMerchSettingList(postData).then((res) => {
        ...
        that.agent = res.result.auth_msg.agent
        that.client = res.result.auth_msg.client
        that.agent_countdown = res.result.auth_msg.agent.values.countdown
        that.client_countdown = res.result.auth_msg.client.values.countdown
        that.merch_auto_reply = res.result.auth_msg.merch_auto_reply
        that.dialog_end = res.result.auth_msg.dialog_end
        that.is_ok = true
      })
