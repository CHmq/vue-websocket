<template>
  ...
  <a-card v-if="is_ok" :bordered="false">
    <div class="title-box">
      ...
      <div class="item-content-box content-input-box">
        ...
        <div>
          ...
          <a-input-number
            ...
            :max="120"
            v-model="queue_limit"
            ref="inputvalue_queue_limit"
            :disabled="is_can_edit === false"
            @change="toChangeInput(1)"/>
          <span>人时，后续新进入对话自动转到留言</span>
          <div class="item-input-tip">* 当前共有{{ agent_data.agent_sum }}个客服人员，最大接待上限{{ agent_data.ceiling_client }}</div>
        </div>
      </div>
      <div class="item-content-box">
        <span class="item-content-title">提示文案：</span><a-textarea class="item-textarea-box" :value="client_queue_setting.values.queueing_settings.intro" @change="toChangeInput(2)" ref="inputvalue_intro" :disabled="is_can_edit===false"/>
      </div>
      <div class="item-content-box">
        <span class="item-content-title">引导留言文案：</span><a-input class="item-content-input-box" :value="client_queue_setting.values.queueing_settings.ticket_intro" @change="toChangeInput(3)" ref="inputvalue_ticket_intro" :disabled="is_can_edit===false"/>
      </div>
...
<script>
import { getMerchSettingList, settingQueueingSetting } from '../../network/Interface/setting'
  ...
  methods: {
    toChangeInput (val) {
      if (val === 1) {
        ...
        this.inputvalue_queue_limit = this.queue_limit
        this.client_queue_setting.values.queueing_settings.queue_limit = this.queue_limit
        console.log('client_queue_setting.values.queueing_settings.queue_limit', this.queue_limit)
      } else if (val === 2) {
    ...
    toCancel () {
      ...
      getMerchSettingList(postData).then((res) => {
        that.client_queue = res.result.queueing.client_queue
        that.client_queue_setting = res.result.queueing.client_queue_setting
        that.queue_limit = res.result.queueing.client_queue_setting.values.queueing_settings.queue_limit
        that.agent_data = res.result.agent
        that.is_ok = true
      })
    },
