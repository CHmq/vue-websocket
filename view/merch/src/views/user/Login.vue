...
import { Login } from '@/network/websocket'
import { agentInfo } from '@/network/Interface/setting'
    ...
    loginPost (values) {
      Login(values).then((res) => {
        ...
        if (res.success) {
          ...
          agentInfo().then((resInfo) => {
            console.log(resInfo)
            const power = resInfo.result.menu
            const avatar = resInfo.result.avatar !== '' ? resInfo.result.avatar : 'http://kefu.xuandev.com/agent/avatar2.jpg'
            sessionStorage.setItem('resInfo', JSON.stringify(resInfo.result))
            sessionStorage.setItem('userName', resInfo.result.name)
            sessionStorage.setItem('userAvatar', avatar)
            sessionStorage.setItem('userPower', JSON.stringify(power))
            this.$store.commit('updateName', resInfo.result.name)
