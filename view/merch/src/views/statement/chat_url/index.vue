<template>
  <div class="datagl datagl2">
    <div class="datagl-title" v-show="dataNewList.length !== 0">着陆页统计</div>
...
import { getLandingPageList } from '../../../network/Interface/chart'
 var data = []
 const columns = [
  { title: '来源',
    width: 200,
...
export default {
  ...
  methods: {
	...
	getDataList (startdate, enddate) {
            const that = this
            getLandingPageList(startdate, enddate).then((res) => {
                if (res.success) {
					...
					this.datashuju = res.result.landing_page_list.data
					// this.datashuju = []
					// 转换百分比
					...
					// this.datashuju.forEach(element => {
						for (const key in this.datashuju[0]) {
							if (key === 'chat_sum' || key === 'dialog_sum' || key === 'client_sum' || key === 'commMiddle' || key === 'page_view' || key === 'poor_rating' || key === 'praise' || key === 'valid_evaluate' || key === 'landing_page' || key === 'result' || key === 'evaluate_rate' || key === 'result_rate') {
								var newkey = key === 'chat_sum' ? '消息数' : key === 'dialog_sum' ? '对话总数' : key === 'client_sum' ? '访客总数' : key === 'commMiddle' ? '中评次数' : key === 'page_view' ? '浏览量' : key === 'poor_rating' ? '差评次数' : key === 'praise' ? '好评次数' : key === 'valid_evaluate' ? '有效评价' : key === 'landing_page' ? '来源' : key === 'result' ? '有效对话数' : key === 'evaluate_rate' ? '有效评价率' : key === 'result_rate' ? '有效的对话率' : ''
								if (key === 'landing_page') {
