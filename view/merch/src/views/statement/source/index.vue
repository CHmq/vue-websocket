<template>
  <div class="datagl datagl2">
    <div class="datagl-title" v-show="dataNewList.length !== 0">来源统计</div>
    <div class="datagl-item">
      <div class="time" v-show="dataNewList.length !== 0">
        ...
        <a-range-picker
          ...
          @change="dateChange"/>
        <a-button type="primary" style="margin-left: 8px" @click="filtrateData">筛选</a-button>
      </div>
      <a-row>
        <a-col :span="24">
          <div class="item-contentlists" style="width:100%">
...
import { getSourceUrlList } from '../../../network/Interface/chart'
var data = []
  console.log(data)
...
const columns = [
  { title: '来源',
    ...
    key: 'name',
    fixed: 'left' },
  { title: '浏览量', dataIndex: 'page_view', key: 'page_view' },
  { title: '有效对话数', dataIndex: 'result', key: 'result' },
  { title: '有效的对话率', dataIndex: 'result_rate', key: 'result_rate' },
  { title: '有效评价', dataIndex: 'valid_evaluate', key: 'valid_evaluate' },
  { title: '有效评价率', ataIndex: 'evaluate_rate', key: 'evaluate_rate' },
  { title: '好评次数', ataIndex: 'praise', key: 'praise' },
  { title: '中评次数', ataIndex: 'commMiddle', key: 'commMiddle' },
  { title: '差评次数', ataIndex: 'poor_rating', key: 'poor_rating' },
  { title: '消息数', ataIndex: 'chat_sum', key: 'chat_sum' },
  { title: '访客总数', ataIndex: 'dialog_sum', key: 'dialog_sum' },
  { title: '对话总数', ataIndex: 'client_sum', key: 'client_sum' }
]
export default {
  ...
  methods: {
	...
	getDataList (startdate, enddate) {
            const that = this
            getSourceUrlList(startdate, enddate).then((res) => {
                if (res.success) {
					...
					this.datashuju = res.result.source_url_list.data
					// this.datashuju = []
					// 转换百分比
					...
					// this.datashuju.forEach(element => {
						for (const key in this.datashuju[0]) {
							if (key === 'chat_sum' || key === 'dialog_sum' || key === 'client_sum' || key === 'commMiddle' || key === 'page_view' || key === 'poor_rating' || key === 'praise' || key === 'valid_evaluate' || key === 'source_url' || key === 'result' || key === 'evaluate_rate' || key === 'result_rate') {
								var newkey = key === 'chat_sum' ? '消息数' : key === 'dialog_sum' ? '对话总数' : key === 'client_sum' ? '访客总数' : key === 'commMiddle' ? '中评次数' : key === 'page_view' ? '浏览量' : key === 'poor_rating' ? '差评次数' : key === 'praise' ? '好评次数' : key === 'valid_evaluate' ? '有效评价' : key === 'source_url' ? '来源' : key === 'result' ? '有效对话数' : key === 'evaluate_rate' ? '有效评价率' : key === 'result_rate' ? '有效的对话率' : ''
								if (key === 'source_url') {
		...
		tubiaoys () {
				this.chart = new G2.Chart({
					...
					forceFit: true,
					height: 300,
					width: document.querySelector('.item-contentlists').offsetWidth,
					padding: [40, 45, 70, 45] // 上右下左
				})
				console.log('图标图标', this.chart, document.querySelector('.item-contentlists'), document.querySelector('.item-contentlists').offsetWidth)
