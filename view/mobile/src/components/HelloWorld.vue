<template>
  <div class="msg">
    ...
    <div class="mobile" :style="'height:' + reversedMessage + 'px'">
      ...
      <div class="msgList2">
        <msgList
          ...
          :imgUrl="imgUrl"
          :isDisabled="Isdisabled"/>
        <div id="msgSend" :class="['msgSend', iphoneXClass]" ref="botFooter" v-show="!show">
          <div style="overflow: auto;background-color: #e6e6e6;width: 100vw;" class="overflowwidtg">
            <div class="msg-tab" id="scrollDivMB" v-show="tabShow">
              <div
                class="tab-item"
                v-for="(item, index) in menuTab"
                :key="index"
...
export default {
  ...
  methods: {
    ...
    ChatsList(page) {
      ...
      this.welcome = list.merch_auth_welcome
    ...
    handleFocus() {
      ...
      if (this.scrollRoll - 1 !== 0 && this.scrollTop === 0) {
      ...
      if (this.scrollTop !== 0) {
        ...
        if (issafariBrowser !== -1) {
          msgHeight.style.height = this.scrollTop + 'px'
        } else {
        ...
        if ((screen.height === 812 && screen.width === 375) || (screen.height === 896 && screen.width === 414)) {
          msgHeight.style.height = window.innerHeight - 44 + 34 + 'px'
        } else {
    ...
    reset() {
      ...
      msgHeight.style.height = this.winHeight - 44+ 'px'
    },
    // 获取焦点
   focusInput() {
      this.iphoneXClass = ''
      const msgSend = document.querySelector('.msgSend')
    // 监听屏幕移动
      this.listenTouchDirection(msgSend, true, this.up, this.right, this.down, this.left)
      // this.listenTouchDirection(msgTab, true, this.up, this.right, this.down, this.left)
      this.btnShow = false
      this.iphoneBottom = false
      // const isIOS = !!navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/) // IOS终端
      const isAndroid  = navigator.userAgent.indexOf('Android') > -1 || navigator.userAgent.indexOf('Linux') > -1 // Android 终端
      // const issafariBrowser = navigator.userAgent.indexOf('Safari') > 0
      if (!isAndroid) {
        msgSend.style.position = 'absolute'
        setTimeout(() => {
        // IphoneX 多出了下面的填充
        if ((screen.height === 812 && screen.width === 375) || (screen.height === 896 && screen.width === 414)) {
          msgSend.style.bottom = '34px'
        }
        if (document.scrollingElement.scrollTop !== 0) {
          this.scrollRoll = document.scrollingElement.scrollTop
        }
    ...
    websocketclose(e) {
      ...
      this.IsSendBtn = true
      this.tabShow = false
      this.$toast('您的对话意外结束，如需唤回客服请刷新页面或退出页面重新进入')
    ...
    right(event){
      // this.scrollAdd()
      document.querySelector('#scrollDivMB').addEventListener('touchstart', this.scrollAdd, false )
      // this.listenTouchDirection(document.querySelector('.msg-tab'), true, this.up, this.right, this.down, this.left)
  ...
  created() {
    ...
    setTimeout(() => {
      this.$nextTick(() => {
        ...
        const container2 = this.$refs.msgList2.$el.querySelector('#talk')
        if (container.scrollHeight !== 0 && container2.scrollHeight === 0) {
          console.log('pc触发了变化')
          container.scrollTop = container.scrollHeight
        } else {
          console.log('触发了变化', container2.scrollHeight)
          container2.scrollTop = container2.scrollHeight
        }
      })
    }, 700)
  ...
  mounted() {
    document.querySelector('.m_msgList_h').addEventListener('touchstart', this.blurInput, false)
